...
function(memory_setup_gtest)
    ...
    elseif(EXISTS "${MEMORY_THIRD_PARTY_DIR}/googletest/CMakeLists.txt")
        ...
        add_subdirectory("${MEMORY_THIRD_PARTY_DIR}/googletest"
...
function(memory_setup_logging)
    ...
    if(EXISTS "${MEMORY_THIRD_PARTY_DIR}/Logging/CMakeLists.txt")
        add_subdirectory("${MEMORY_THIRD_PARTY_DIR}/Logging"
                         "${CMAKE_BINARY_DIR}/ThirdParty/Logging_build" EXCLUDE_FROM_ALL)
    else()
        include(FetchContent)
        FetchContent_Declare(
            Logging
            GIT_REPOSITORY https://github.com/KhwarizmiAnalytix/Logging.git
            GIT_TAG main
        )
        FetchContent_MakeAvailable(Logging)
    endif()
    if(NOT TARGET Logging::Logging)
        message(FATAL_ERROR "Memory requires Logging::Logging but it could not be resolved "
                            "(git submodule update --init ThirdParty/Logging, or network "
                            "access for FetchContent)")
    endif()
endfunction()
...
function(memory_setup_profiler)
    ...
    if(EXISTS "${MEMORY_THIRD_PARTY_DIR}/Profiler/CMakeLists.txt")
        add_subdirectory("${MEMORY_THIRD_PARTY_DIR}/Profiler"
                         "${CMAKE_BINARY_DIR}/ThirdParty/Profiler_build" EXCLUDE_FROM_ALL)
    else()
        include(FetchContent)
        FetchContent_Declare(
            Profiler
